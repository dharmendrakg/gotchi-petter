...
const POLYGON_RPC_HOST =
  process.env.POLYGON_RPC_HOST || "https://polygon-rpc.com/";
  ...
  new Promise((resolve, reject) => {
    ...
    fetch(POLYGON_GAS_STATION_HOST)
      ...
      .then((gasData) => {
        if (gasData["error"])
          reject(
            new Error(`Polygon gas station error: ${gasData.error.message}`)
          );
        else if (typeof gasData[GAS_SPEED] == "undefined")
          reject(
            new Error(
              `Polygon gas station response does not include any data for gas speed '${GAS_SPEED}' (response=${JSON.stringify(
                gasData
              )})`
            )
          );
...
const setTransactionGasToMarket = async (tx) =>
  Object.assign(
    {
      gasLimit: await web3.eth.estimateGas(tx),
      maxPriorityFeePerGas: Math.ceil(
        convertGweiToWei(
          (await getCurrentGasPrices())[GAS_SPEED].maxPriorityFee
        )
      ),
    },
    tx
...
const signPetTransaction = (unsignedTransaction) =>
  web3.eth.accounts.signTransaction(unsignedTransaction, PETTER_WALLET_KEY);
const sendPetTransaction = (signedTransaction) =>
...
const notifyReceipt = (receipt) =>
  log(
    `Obtained receipt for transaction (blockNumber=${receipt.blockNumber}, gasUsed=${receipt.gasUsed}, effectiveGasPrice=${receipt.effectiveGasPrice})`
  );
...
const getSecondsSinceLastPet = (gotchi) =>
  Math.floor(Date.now() / 1000) - gotchi.lastInteracted;
const isReadyToBePet = (gotchi) =>
  getSecondsSinceLastPet(gotchi) > SECONDS_BETWEEN_PETS;
...
const filterPettableGotchiIds = async (unfilteredIds) => {
  ...
  for (id of unfilteredIds) {
    ...
    log(
      `Found gotchi: (id=${gotchi.tokenId}, lastInteracted=${new Date(
        gotchi.lastInteracted * 1000
      )})`
    );
    isReadyToBePet(gotchi)
      ? pettableIds.push(id)
...
async function petAavegotchis(ids) {
  ...
  try {
    var petTransaction = await setTransactionGasToMarket(
      await createPetTransaction(ids)
    );
  } catch (err) {
    return Promise.reject(`Error creating transaction: ${err.message}`);
  }
  log(
    `Creating pet transaction: (from=${petTransaction.from}, to=${petTransaction.to}, gasLimit=${petTransaction.gasLimit}, maxPriorityFeePerGas=${petTransaction.maxPriorityFeePerGas})`
  );
  const estimatedGasCostMatic = convertWeiToMatic(
    petTransaction.gasLimit *
      (petTransaction.maxPriorityFeePerGas +
        convertGweiToWei((await getCurrentGasPrices()).estimatedBaseFee))
  );
  log("Estimated gas cost is ~" + estimatedGasCostMatic.toFixed(6) + " MATIC");
  if (estimatedGasCostMatic > GAS_COST_LIMIT_MATIC) {
    log(
      "ABORTED: Estimated gas cost exceeds limit. GAS_COST_LIMIT_MATIC=" +
        GAS_COST_LIMIT_MATIC
    );
  } else {
...
const loop = () =>
  filterPettableGotchiIds(GOTCHI_IDS).then(petAavegotchis).catch(log);
